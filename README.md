...
The Progress route is still a placeholder. Upload, parsing, chunking, retrieval, topic extraction, grounded question generation, and the study session are in place.
...
- **Study.** `/study` presents one due question at a time (default 10), weakest topics first, then never-seen items. `/study?document=<id>` restricts to one document; topic and question query params narrow further. Short answers use a textarea; multiple choice uses A–D rows. A source disclosure expands the cited passage without affecting grading. Keyboard: `⌘/Ctrl Enter` submits, `1–4` select options, `S` toggles the source, `Enter`/`Space` advances after feedback, `Esc` ends. Submit stores the answer as a stub attempt and shows a placeholder — the AI grader is not in the loop yet. Nothing-due offers study ahead; the closing screen lists recorded answers and topics.
