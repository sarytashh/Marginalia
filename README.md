...
- **Topics and questions.** After the search index is ready, Marginalia samples passages across the document and asks the chat model for specific topics (name + one-sentence summary). For each topic it retrieves the nearest chunks, then asks the model to write 4–6 questions using **only those passages**. The system prompt forbids using general knowledge; questions that cite unknown chunks are dropped before insert. Each question stores `source_chunk_ids` and a `review_state` row due immediately. Generation runs across topics with a concurrency limit of 2.
- **Document detail.** `/documents/[id]` is the course-reader spread: title and metadata, the processing pipeline while work is in progress, a numbered topic index, and an expandable question archive. Each question shows its type, difficulty, source page, reference answer, and a passage excerpt. Primary action starts study for that material; Generate more writes another grounded batch.
