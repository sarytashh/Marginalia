...
- [docs/design.md](design.md) — the design spec: screens, visual direction, every state
- [docs/prompts.md](prompts.md) — sequenced prompts, copy-paste in order
- [docs/cursor-rules.md](cursor-rules.md) — paste into the project's `.cursor/rules/` so quality stays consistent
...
- **Commit per logical change, not one giant dump.** A history of 40 meaningful commits over weeks
  ...
  [docs/prompts.md](prompts.md) instruct the agent to commit and push after each step.
...
Each of these maps to a prompt in [prompts.md](prompts.md). Stop and actually use the app
